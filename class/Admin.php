<?php
	class Admin extends Db{
		public function admin_login($un,$pw){
			 $pw = md5(sha1($pw));
			$this->query = $this->handeller->query("SELECT * FROM `users` WHERE (username = '$un' AND password = '$pw')");
			$row = $this->query->rowCount();
			
				if($row > 0){
					$this->data = array($this->user_ip(),$un,$pw);

					$this->query2 = $this->handeller->prepare("UPDATE `users` SET ip_address = ? WHERE (username = ? AND password = ?)");
					$this->query2->execute($this->data);

					$this->query3 = $this->handeller->query("SELECT * FROM `users` WHERE (username = '$un' AND password = '$pw')");
					$row = $this->query3->fetch(PDO::FETCH_ASSOC);
					$_SESSION['admin'] = $row;
						$this->url_return(array('index.php','Successfully logged in','success'));
					}else{
						$this->url_return(array('login.php','Invalid Username or Password','error'));
						}
			}
